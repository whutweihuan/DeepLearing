...
# 该程序加入迁移学习的预处理 处理 HWDB2.0~2.2中文数据集　crnn + ctc
import Levenshtein
import torch
...
class Encoder(nn.Module):
    ...
    def forward (self, input):
        ...
        encoder_outputs = F.softmax(encoder_outputs, 2)
        return encoder_outputs


# 将文字标签转化为和图片一样大的概率矩阵
class LabelModel(nn.Module):
    def __init__ (self, dictionary_size = word_lang.size(), hidden_size = 256):
        super(LabelModel, self).__init__()
        self.dictionary_size = dictionary_size
        self.hidden_size = hidden_size

        self.embed = nn.Embedding(dictionary_size, hidden_size)
        self.rnn = nn.LSTM(hidden_size, hidden_size, bidirectional = True)
        self.liner = nn.Linear(2 * hidden_size, dictionary_size)

    def forward (self, labels):
        embed_label = self.embed(labels)
        # print(embed_label.shape)
        # print(labels.shape)
        out, _ = self.rnn(embed_label.permute(1, 0, 2))
        t, b, h = out.size()
        out = out.view(t * b, h)
        # print(out.shape)
        out = self.liner(out).view(t, b, -1)
        out = F.softmax(out, 2)

        black_tensor = torch.zeros(1, b, out.shape[2]).to(device)
        black_tensor[:, :, 2] = 1
        out_temp = torch.cat((out, black_tensor), 0)

        extent_matrix = out_temp
        # print(a.shape)

        nrepeat = (IMG_WIDTH // 4 + 1) // (t + 1)
        nblack = (IMG_WIDTH // 4 + 1) % (t + 1)

        # 添加for循环重复
        for i in range(nrepeat - 1):
            extent_matrix = torch.cat((extent_matrix, out_temp), 0)
        for i in range(nblack):
            extent_matrix = torch.cat((extent_matrix, black_tensor), 0)

        return extent_matrix
...
loss_mse = torch.nn.MSELoss(reduction = 'mean',size_average=True)
        ...
        for iter, (x, y) in enumerate(train_dataloader):
            ...
            if (iter + 1) % 10 == 0:
                logger.info("[{}/{}][{}/{}] loss: {}".format(epoch + 1, N_EPOCH, iter + 1, len(train_dataloader),
                                                             loss_total / 10))
                loss_total = .0
        torch.save(encoder.state_dict(), '07-encoder_ctc.pt')
...
def pre_train ():
    train_dataloader = DataLoader(td, batch_size = BATCH, shuffle = True)
    try:
        encoder.load_state_dict(torch.load('07-encoder_ctc.pt'))
        print("读取模型成功，开始训练")
    except:
        print("未能读取模型，重新开始训练")
    encoder.train()
    for epoch in range(N_EPOCH):
        ...
        for iter, (x, y) in enumerate(train_dataloader):
            ...
            label = word_lang.batchlabels2vec(y)
            x,label = x.to(device),label.to(device)
            extent_matrix = labelmodel(label)
            # print(extent_matrix.shape)
            # print(extent_matrix)

            encoder_output = encoder(x)  # seq * batch * dic_len
            matrix_loss = loss_mse(encoder_output,extent_matrix)
            ...
            ctc_loss = loss_fn(encoder_output, label, preds_size, label_len)
            print(matrix_loss.item(),ctc_loss.item())
            loss = ctc_loss
            encoder.zero_grad()
            loss.backward()
            loss_total += loss.item()
